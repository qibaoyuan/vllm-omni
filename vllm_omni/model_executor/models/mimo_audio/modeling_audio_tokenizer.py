...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if hidden_states.is_cuda and is_flash_atth_available is True:
            ...
            # === CPU / Eager 模式下回退实现 ===
            # 注意此处默认 seq_len 为 [bsz] 的每个样本长度
            attn_output = torch.zeros_like(hidden_states)
            start = 0
            for slen in seq_len.tolist():
                end = start + slen

                # 针对当前样本切片
                q = query_states[start:end].transpose(0, 1)  # [num_heads, slen, head_dim]
                ...
                # 展平到原始 embedding 维度
                attn_output[start:end] = attn_out.transpose(0, 1).reshape(slen, self.embed_dim)
                start = end
