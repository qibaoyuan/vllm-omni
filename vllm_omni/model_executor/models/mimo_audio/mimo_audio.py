...
class MiMoAudioForConditionalGeneration(
    ...
    def __init__(
        ...
        if self.model_stage == "fused_thinker_talker":
            ...
            # Enable postprocess hook: we trigger MiMo local decoding after sampling
            # (outside vLLM CUDA graph capture) and cache embeddings per request.
    ...
    def fused_thinker_talker_decode_one_step(
        ...
        # Decode-time embedding merge (graph-external): if the token is <|empty|>,
        # merge cached audio embedding generated by previous postprocess.
        try:
            if input_ids is not None and input_ids.numel() == 1 and self.fused_thinker_talker is not None:
                empty_token_id = int(self.fused_thinker_talker.empty_token_id)
                if int(input_ids.view(-1)[0].item()) == empty_token_id:
                    req_id = info_dict.get("req_id") or info_dict.get("request_id")
                    if not isinstance(req_id, str) or not req_id:
                        req_id = "default"
                    cached = getattr(self.fused_thinker_talker, "_cached_new_audio_emb_by_req", {}).get(req_id)
                    if cached is not None and input_embeds is not None:
                        if cached.dim() == 1:
                            cached = cached.view(1, -1)
                        input_embeds = input_embeds + cached.to(device=input_embeds.device, dtype=input_embeds.dtype)
        except Exception:
            # Best-effort merge; do not break main decode path.
            pass
    ...
    def fused_thinker_talker_postprocess(self, hidden_states: torch.Tensor, **info_dict: object):
        """Per-request postprocess (compat path). Prefer batched version via postprocess_batch."""
        updates = self.postprocess_batch(
            hidden_states=hidden_states,
            req_infos_list=[dict(info_dict)],
            # For per-request slices, treat as a single request starting at 0.
            query_start_loc=torch.tensor([0, hidden_states.shape[0]], device=hidden_states.device),
            num_scheduled_tokens_np=np.array([hidden_states.shape[0]], dtype=np.int32),
        )
        if isinstance(updates, list) and updates:
            return updates[0]
        if isinstance(updates, dict):
            # single request id unknown here; return first update
    ...
    def postprocess_batch(
        ...
        *,
        ...
        """Batched postprocess: run local decoding for requests sampled <|empty|>.

        Args:
            hidden_states: Flattened hidden states for the whole step [num_tokens, hidden].
            req_infos_list: Per-request info dicts (aligned with current batch order).
            query_start_loc: Prefix sum offsets for each request in flattened token space.
            num_scheduled_tokens_np: Number of scheduled tokens per request.

        Returns:
            List of update dicts aligned with req_infos_list.
        """
        
        
        
        updates_list: list[dict] = [{} for _ in range(len(req_infos_list))]
        ...
        for i, info in enumerate(req_infos_list):
            try:
                sampled_token_id = info.get("sampled_token_id", None)
                ...
                req_id = info.get("req_id") or info.get("request_id")
                if not isinstance(req_id, str) or not req_id:
                    req_id = str(i)
                ...
                if end <= start:
                    continue
                hs_last = hidden_states[end - 1 : end].unsqueeze(0)  # [1, 1, H]

                selected_req_indices.append(i)
                selected_req_ids.append(req_id)
                hs_list.append(hs_last)
            except Exception:
                continue
        ...
        batch_hs = torch.cat(hs_list, dim=0)  # [B, 1, H]
        try:
        ...
        # Cache embeddings per request for next-step decode merge; expose codec codes.
        try:
            if new_audio_emb is not None:
                for j, req_id in enumerate(selected_req_ids):
                    self.fused_thinker_talker._cached_new_audio_emb_by_req[req_id] = new_audio_emb[j]
        except Exception:
            pass

        try:
            if next_speech_tokens is not None:
                next_speech_tokens_cpu = next_speech_tokens.detach().to("cpu").contiguous()
                for j, req_idx in enumerate(selected_req_indices):
                    code_slice = next_speech_tokens_cpu[j : j + 1]
                    updates_list[req_idx]["code"] = code_slice
        ...
        **kwargs: object,
        ...
        if self.model_stage == "fused_thinker_talker":
            ...
            return OmniOutput(
                ...
                # Codec codes are generated in postprocess and injected into runner's pooler_output,
                # not returned from the model forward path (to keep CUDA graph stable).
                multimodal_outputs={},
